...
__device__ __forceinline__ BFloat16x2 fast_exp(const BFloat16x2& x) {
    hip_bfloat16 x1 = __low2bfloat16(x.data);
    hip_bfloat16 x2 = __high2bfloat16(x.data);
...
__device__ void fast_softmax(hip_bfloat16* input, hip_bfloat16* output, int size) {
    ...
    hip_bfloat16 max_val = input[0];
    for (int i = 1; i < size; i++) {
        max_val = __hmax(max_val, input[i]);
    }
    
    // Compute exponentials and sum
    hip_bfloat16 sum = BF16_ZERO;
    ...
    // Normalize
    hip_bfloat16 inv_sum = fast_rcp(sum);
    for (int i = 0; i < size; i++) {
...
__device__ __forceinline__ hip_bfloat16 inclusive_scan_sum(hip_bfloat16 val) {
    for (int offset = 1; offset < WARP_SIZE; offset *= 2) {
        hip_bfloat16 temp = __shfl_up(val, offset);
...
__device__ __forceinline__ hip_bfloat16 exclusive_scan_sum(hip_bfloat16 val) {
    hip_bfloat16 inclusive = inclusive_scan_sum(val);
    hip_bfloat16 previous = __shfl_up(inclusive, 1);
...
__device__ hip_bfloat16 reduce_sum(hip_bfloat16 val) {
    __shared__ hip_bfloat16 shared_mem[BLOCK_SIZE];
...
template __device__ hip_bfloat16 reduce_sum<256>(hip_bfloat16);
template __device__ hip_bfloat16 reduce_sum<512>(hip_bfloat16);
template __device__ hip_bfloat16 reduce_max<256>(hip_bfloat16);
template __device__ hip_bfloat16 reduce_max<512>(hip_bfloat16);
...
__device__ void compute_attention_weights(
    ...
    int seq_len, int head_dim, hip_bfloat16 scale, bool causal_mask
) {
    ...
    if (row < seq_len && col < seq_len) {
        // Compute dot product
        hip_bfloat16 score = fast_math::dot_product(
